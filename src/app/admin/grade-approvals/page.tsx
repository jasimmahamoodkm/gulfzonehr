...
export default function GradeApprovalsPage() {
  ...
  const typeBadge = (t: ChangeRequest['request_type']) => {
    ...
    if (t === 'demotion') return <span className="inline-flex items-center gap-1 text-destructive bg-destructive/15 px-2 py-0.5 rounded text-xs font-medium"><TrendingDown size={12} /> Demotion</span>;
    return <span className="inline-flex items-center gap-1 text-secondary-foreground bg-secondary px-2 py-0.5 rounded text-xs font-medium"><ArrowRight size={12} /> Lateral</span>;
  };
  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Promotion / Demotion Approvals</h1>
        ...
        <div className="space-y-4">
          {loading ? (
            <Card className="p-8 text-center text-muted-foreground">Loading pending requests…</Card>
          ) : requests.length === 0 ? (
            ...
            requests.map((r) => {
              ...
              return (
                <Card key={r.id} className="p-6 border-l-4 border-l-primary">
                  <div className="flex items-start justify-between flex-wrap gap-2">
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-foreground">{empName}</h3>
                  ...
                  <div className="mt-4 space-y-2 text-sm">
                    {r.change_grade && (
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-foreground w-24">Grade</span>
                        <span className="text-muted-foreground">{r.curr?.name || '—'}</span>
                        <ArrowRight size={14} className="text-muted-foreground" />
                        <span className="text-foreground font-medium">{r.req?.name || '—'}</span>
                      </div>
                    )}
                    {r.change_salary && (
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-foreground w-24">Salary</span>
                        <span className="text-muted-foreground">{fmt(r.current_salary, r.currency || 'AED')}</span>
                        <ArrowRight size={14} className="text-muted-foreground" />
                        <span className="text-foreground font-medium">{fmt(r.requested_salary, r.currency || 'AED')}</span>
                      </div>
                    )}
                    {r.change_benefits && r.benefit_changes && (
                      <div className="flex items-start gap-2">
                        <span className="font-medium text-foreground w-24">Benefits</span>
                        <div className="flex-1 space-y-0.5">
                          {r.benefit_changes.map((b, i) => (
                            <div key={i} className="text-foreground">
                              <span className={`text-xs px-1.5 py-0.5 rounded mr-2 ${b.action === 'remove' ? 'bg-destructive/15 text-destructive' : 'bg-green-100 text-green-700'}`}>
                                {b.action === 'remove' ? 'Remove' : 'Add/Update'}
                              </span>
                              {b.benefit_type}
                              {b.action !== 'remove' && b.benefit_value != null && (
                                <span className="text-muted-foreground"> — {b.value_type === 'percentage' ? `${b.benefit_value}% of basic` : fmt(b.benefit_value, r.currency || 'AED')}</span>
                              )}
                            </div>
                          ))}
                        </div>
                    ...
                    {r.reason && (
                      <div className="flex items-start gap-2">
                        <span className="font-medium text-foreground w-24">Reason</span>
                    ...
                    {isOwn ? (
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <div className="lg:col-span-2">
                          <label className="block text-sm font-medium text-foreground mb-1">Note (required to reject)</label>
                          <textarea
                            value={notes[r.id] || ''}
